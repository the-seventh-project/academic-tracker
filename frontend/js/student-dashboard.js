...
window.onload = async () => {
    ...
    if (studentNameEl) {
        ...
        // Make name clickable to navigate to account information
...
async function loadWeather() {
    try {
        const response = await fetch(`${API_URL}/campus-weather`);
...
async function loadGPA() {
    try {
        const response = await fetch(`${API_URL}/calculate-gpa/${currentUser.id}`);
        const data = await response.json();
        ...
        const coursesResponse = await fetch(`${API_URL}/courses/${currentUser.id}`);
...
async function loadCourses() {
    try {
        const response = await fetch(`${API_URL}/courses/${currentUser.id}`);
            ...
            tbody.innerHTML = `
                <tr>
                    <td colspan="6" class="text-center text-muted">
                        No courses yet. <a href="add-course.html">Add your first course</a>
                    </td>
                </tr>
            `;
            return;
...
async function deleteCourse(courseId, courseCode) {
    ...
    try {
        const res = await fetch(`${API_URL}/delete-course/${courseId}`, { method: 'DELETE' });
        if (res.ok) {
            try { localStorage.setItem('courses_updated', String(Date.now())); } catch (e) { }
            await loadDashboardData();
        } else {
            const txt = await res.text();
            alert('Failed to delete course: ' + txt);
        }
    } catch (err) {
...
function viewCourse(courseId) {
    ...
    window.location.href = `course-detail.html?id=${courseId}&view=1`;
}
...
async function fetchAssessmentsBySemester(assessmentType) {
    ...
    for (const course of allCourses) {
        ...
        try {
            const response = await fetch(`${API_URL}/assessments/${course.course_id}`);
