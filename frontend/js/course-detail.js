...
window.onload = async () => {
    ...
    if (!courseId) {
        ...
        window.location.href = fromAdmin ? 'admin-overview.html' : 'student-dashboard.html';
        return;
    }
    ...
    document.getElementById('backToDashboard')?.addEventListener('click', (e) => {
        ...
        if (totalWeight !== 0 && totalWeight !== 100) {
            ...
            const backUrl = fromAdmin ? 'admin-overview.html' : 'student-dashboard.html';
...
async function loadCourseDetails() {
    try {
        ...
        const courseResponse = await fetch(`${API_URL}/course/${courseId}`);
        ...
        // Fetch assessments
        const response = await fetch(`${API_URL}/assessments/${courseId}`);
...
document.getElementById('editCourseBtn')?.addEventListener('click', async () => {
    try {
        const resp = await fetch(`${API_URL}/course/${courseId}`);
        if (!resp.ok) return alert('Failed to load course for editing');
        ...
        document.getElementById('editCourseCredits').value = course.credit_hours || 0;
...
document.getElementById('editCourseForm')?.addEventListener('submit', async (e) => {
    ...
    const payload = {
        ...
        credit_hours: Number(document.getElementById('editCourseCredits').value) || 0,
        semester: document.getElementById('editCourseSemester').value.trim()
    };

    if (!payload.course_code || !payload.course_name) {
        return alert('Course code and name are required');
    }

    try {
        const resp = await fetch(`${API_URL}/update-course/${courseId}`, {
            method: 'POST',
        ...
        if (resp.ok && result.success) {
            ...
            alert('Failed to update course: ' + (result.error || result.message || 'Unknown'));
        }
    } catch (err) {
...
document.getElementById('assessmentForm')?.addEventListener('submit', async (e) => {
    ...
    try {
        let url = `${API_URL}/add-assessment`;
        let method = 'POST';

        if (currentAssessmentId) {
            url = `${API_URL}/update-assessment/${currentAssessmentId}`;
            // The backend might expect PUT or POST for updates. Based on app.py, it's likely POST or PUT.
            // Let's check app.py if needed, but usually POST is safe if designed that way.
            // Wait, I recall the backend route for update might be /update-assessment/<id>.
            // Let's assume POST for now as per common Flask patterns unless specified otherwise.
        }
...
async function deleteAssessment(id) {
    ...
    try {
        ...
        const response = await fetch(`${API_URL}/delete-assessment/${id}`, {
            method: 'DELETE'
        });
