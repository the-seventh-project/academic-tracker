...
window.onload = async () => {
    ...
    try {
        const profileResponse = await fetch(`${API_URL}/api/students/${user.id}/profile`);
...
async function loadProgressReport(userId) {
    try {
        // Fetch GPA data
        const gpaResponse = await fetch(`${API_URL}/calculate-gpa/${userId}`);
        let courseGrades = [];
        if (gpaResponse.ok) {
...
async function saveInformation() {
    ...
    // Try to save to API
    try {
        const response = await fetch(`${API_URL}/api/students/${user.id}/profile`, {
            method: 'PUT',
